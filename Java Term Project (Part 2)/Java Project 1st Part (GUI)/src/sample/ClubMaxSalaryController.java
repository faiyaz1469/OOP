...
public class ClubMaxSalaryController {
    ...
    public ClubMaxSalaryController() throws Exception {
    }

    public void showClubMaxSalaryPlayersInfo(ActionEvent actionEvent) throws Exception {
        String userClName = userClubName.getText();
        List<Player> plrList = pList.getMaxSalaryClubList(userClName);
        boolean ans = plrList.isEmpty();
        if (ans == false) {
            main.showClubMaxSalaryPlayerInfoPage(userClName);
        }else{
